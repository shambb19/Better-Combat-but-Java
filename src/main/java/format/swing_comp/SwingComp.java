...
public class SwingComp<E extends JComponent> {
    ...
    public SwingComp<E> enabledIf(boolean condition) {
    ...
    public SwingComp<E> withAction(Consumer<E> action) {
        if (component instanceof AbstractButton btn)
            btn.addActionListener(e -> action.accept(component));
    ...
    public static SwingComp<JButton> button(String name, Runnable actionListener) {
        ...
        return SwingComp.modifiable(button).round().withAction(actionListener);
    }
    ...
    public static SwingComp<JLabel> label(String text) {
        return new SwingComp<>(new JLabel(text));
    }
    ...
    public static SwingComp<JTextField> field() {
        return new SwingComp<>(new JTextField());
    }
    ...
    @SuppressWarnings("unchecked")
    public static <T extends JComponent> SwingComp<T> cloned(T component) {
        if (component == null) {
            throw new IllegalArgumentException("cloned in SwingComp: null input");
        }
        ...
        try (ByteArrayOutputStream byteOutput = new ByteArrayOutputStream();
            ...
            try (ByteArrayInputStream byteInput = new ByteArrayInputStream(byteOutput.toByteArray());
                ...
                return new SwingComp<>(copy);
            }

        } catch (Exception e) {
            Logger.getAnonymousLogger().severe("cloned in SwingComp: could not copy component: " + e);
            Message.error("cloned in SwingComp: could not copy component", e);
